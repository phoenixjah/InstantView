...
@synthesize panRecognizer, longPressRecognizer;
    ...
    if ((recognizer.state == UIGestureRecognizerStateBegan
        ...
        if (fabsf(translation.x) >= commitEditingLength) {
            if (self.addingCellState == JTTableViewCellEditingStateMiddle) {
                self.addingCellState = translation.x > 0? JTTableViewCellEditingStateRight:JTTableViewCellEditingStateMiddle;
            }
        } else {
            if (self.addingCellState != JTTableViewCellEditingStateMiddle) {
...
- (void)longPressGestureRecognizer:(UILongPressGestureRecognizer *)recognizer {
    ...
    //make sure it cannot move item to top
//    if (indexPath.row == 0) {
//        NSLog(@"cannot move to here");
...
- (JTTableViewGestureRecognizer *)enableGestureTableViewWithDelegate:(id)delegate {
    if (! [delegate conformsToProtocol:@protocol(JTTableViewGestureEditingRowDelegate)]
